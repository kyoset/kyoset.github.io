        ...
        res.on('end', () => {
            _axios.post(urlx, {}, {
                ...
                if(dlurl.downloadUrl === undefined) {
                    console.log('🔥  ' + _colors.red('Link Download File LIMIT, Try Another URL'));
                    return cb();
                }
                console.log('🎁  ' + _colors.yellow('Start Download From URL : ' + dlurl.downloadUrl));
                dLoad(dlurl, cb);
            }).catch(() => {
                console.log('❎  ' + _colors.orange('File Not Found!, Try Another URL'));
                return cb();
            });
...
const dLoad = (url, cb) => {
    ...
    req.on('response', (res) => {
        ...
        res.on('end', () => {
            loadbar.stop();
            file.close();
            console.log('✅  ' + _colors.green('Success Download : ' + filename));
            cb()
        });
        res.on('error', () => {
            _fs.unlink(filename);
